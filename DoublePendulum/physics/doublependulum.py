...
class DoublePendulum:
    ...
    def _validate_state_arr(self):
        """
        If down is theta=0, and our goal is up (theta=pi), lets wrap at
        -pi/2 / 3pi/2
        """
        for i in [1,2]:
            changed = False
    ...
    def step(self, F):
        now_time = self.t
        stop_time = now_time+self.dt
        ts = []
        while self.t < stop_time:
            solver = self._RK_step(F)
            ts.append(solver.t)
        interpolant = solver.dense_output()
        self.x = interpolant(stop_time)
        self._validate_state_arr()
        self.t = stop_time
    ...
    def transform_cartesian(self, do_vels=False):
        ...
        if do_vels:
            ...
            vy2 = vy1+self.l2*np.sin(self.x[2])*self.x[5]
